...
{
	bool verbose;
	const char FPcodename[3][5] = { "x87", "SSE", "SSE2" };
	int i, j, k;
	double M, e;
	verbose = false;
	InitAll();
...
#ifdef _MSC_VER
 #ifdef _M_IX86
	printf(" MSC_ver %i legacy x86 SSE %i (%s)", _MSC_FULL_VER,FPcodename[__M_IX86_FP],_M_IX86);
 #else
#ifdef _M_X64
	printf(" MSC_ver %i x64 SSE %i ", _MSC_FULL_VER, _M_X64);
#else
			  ...
			  if (i<NFUNCT)
			  {
			    ...
			    {		
			    ...
			    else
					{
						...
						{
							if (argc == 4) MasterTest(k, e, M, true, verbose);
							else
							{
							for (i = 1; i< argc-1;  i++)
							{
